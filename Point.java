...
public class Point {
    ...
    private final int x;
    private final int y;
    public Map<String, Double> mapDistances = new HashMap<>();
    public static Map<String, Double> pathsToB = new HashMap<>();

    public String getkeyPathsToB(Point point) {
        Double penultimateValue =  Collections.max(pathsToB.values());
        return getKeyOfMap(pathsToB, penultimateValue);
    }
    public static void removeFromPathsToB (String pointName){
        pathsToB.remove(pointName);
    }
    ...
    public void pathsToB(Point point) {
        pathsToB.put(point.getName(), calculateDistance(point));
    }
}
